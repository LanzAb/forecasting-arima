...
### 2026-09-24: Bug ketemu dari laporan user — pita interval kepercayaan kolaps jadi nol untuk model tanpa AR/MA

User melaporkan (lewat screenshot halaman Proses Forecasting) satu hasil peramalan dengan model **ARIMA(0,0,0)** yang grafiknya rata sempurna: garis forecast, batas atas, dan batas bawah semuanya berimpit di angka yang sama persis untuk 6 bulan ke depan. Ditelusuri sampai ke `Forecaster::hitungInterval()`.

**Akar masalah**: rumus interval kepercayaan Box-Jenkins yang benar (Wei, *Time Series Analysis*, dikutip di komentar kelasnya sendiri) adalah `Var(e_h) = sigma^2 * SUM_{j=0}^{h-1} psi_j^2`, dengan **psi_0 = 1 selalu** (bagian dari definisi representasi MA-tak-hingga). Kode lama menjumlahkan `psi_1..psi_h` saja lewat `psiWeights()` (yang memang sengaja `unset($psi[0])` karena psi_0 dianggap "trivial") tapi **psi_0 tidak pernah ditambahkan balik di `hitungInterval()`** — jadi utk h=1, lebar interval yang seharusnya `sigma * sqrt(psi_0^2) = sigma` malah dihitung dari `psi_1` doang, dan seterusnya bergeser satu index utk h berikutnya.

Dampaknya dua tingkat:
1. **Untuk model apapun** (bukan cuma yang bermasalah) — seluruh pita interval kepercayaan yang pernah ditampilkan sistem ini dari awal Fase 3 lebih sempit dari seharusnya, karena kontribusi psi_0=1 (guncangan bulan pertama, komponen ketidakpastian terbesar) tidak pernah dihitung.
2. **Untuk model tanpa AR maupun MA sama sekali** (ARIMA(p,0,q) dengan p=q=0, alias murni konstanta) — `psi_1` sampai `psi_h` semuanya nol karena tidak ada koefisien AR/MA yang direkursikan, sehingga TANPA psi_0=1 jumlahnya benar-benar nol dan intervalnya kolaps persis jadi titik forecast-nya sendiri. Ini yang kelihatan sebagai "garis lurus" di laporan user.

**Kenapa lolos dari validasi manual sebelumnya**: catatan Fase 3 (`2026-09-21`) menyebut forecast AR(1) murni "divalidasi hitung tangan, cocok persis" — ternyata perhitungan tangan itu sendiri ikut melewatkan psi_0=1 di kasus h=1 (kesalahan konsep yang sama terulang di dua tempat independen, jadi kelihatan "cocok" padahal berdua-duanya salah). Baru ketahuan sekarang karena kasus ARIMA(0,0,0) membuat efeknya jadi ekstrem (nol persis, bukan cuma sedikit sempit) sehingga kelihatan jelas di grafik.

**Perbaikan**: `Forecaster::hitungInterval()` sekarang mulai menjumlahkan dari `$sumPsiKuadrat = 1.0` (psi_0^2) sebelum menghitung se(h=1), baru menambah `psi[h]^2` di akhir tiap iterasi untuk iterasi berikutnya — cocok persis pola `SUM_{j=0}^{h-1}`. Test lama `test_forecast_ar1_interval_kepercayaan_sesuai_psi_weight_ar` diperbaiki angka pembandingnya (dulu ikut salah). Test baru `test_interval_h1_selalu_sigma_dikali_z_berapapun_modelnya` khusus menutup kasus konstanta murni: interval harus tetap punya lebar (bukan nol) dan sama besar di semua horizon (karena psi_1 dst memang nol, tapi psi_0=1 tetap ada).

**Yang TIDAK terdampak**: safety stock dan simulasi backtest (`TargetProduksiPlanner`, `SimulationRunner`) tidak memakai interval Forecaster ini — keduanya punya rumus sigma_e sendiri yang terpisah. Bug ini murni memengaruhi pita interval kepercayaan yang ditampilkan di halaman Proses Forecasting, tidak memengaruhi rekomendasi stok/produksi manapun. `php artisan test` penuh setelah perbaikan: **361 passed, 0 failed**.
